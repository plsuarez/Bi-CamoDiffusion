...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768, mask_chans=0):
        ...
        self.register_buffer("_laplacian_k", lap, persistent=False)

 
    def forward(self, x, mask=None, edge=None, lambda_inj: float = 0.0, edge_filter: str = "none"):

        feat = self.proj(x)

         
        if edge is not None and lambda_inj is not None and float(lambda_inj) > 0.0:
            e = edge
            ...
            e = e.to(device=feat.device, dtype=feat.dtype)
             
            e = F.interpolate(e, size=feat.shape[-2:], mode='nearest')
            if edge_filter == "laplacian":
                k = self._laplacian_k.to(device=feat.device, dtype=feat.dtype)
                e = torch.abs(F.conv2d(e, k, padding=1))

            e = e.clamp(0.0, 1.0)
            e = e.repeat(1, feat.shape[1], 1, 1)   
            feat = feat + float(lambda_inj) * e
...
class PyramidVisionTransformerImpr(nn.Module):
    ...
    # --- NEW: add edge/lambda_inj/edge_filter only to stage-1 call, preserve rest ---
    def forward_features(self, x, timesteps, cond_img, edge=None, lambda_inj: float = 0.0, edge_filter: str = "none"):
        ...
        # stage 1
        x, H, W = self.patch_embed1(cond_img, x, edge=edge, lambda_inj=lambda_inj, edge_filter=edge_filter)  # <-- NEW
    ...
    def forward(self, x, timesteps, cond_img, edge=None, lambda_inj: float = 0.0, edge_filter: str = "none"):
        x = self.forward_features(x, timesteps, cond_img, edge=edge, lambda_inj=lambda_inj, edge_filter=edge_filter)
        return x
...
class net(nn.Module):
    def __init__(self, class_num=2, mask_chans=0, lambda_inj: float = 0.0, edge_filter: str = "none", **kwargs):
        super(net, self).__init__()
        self.class_num = class_num

        # ---   read from YAML model.params ---
        self.lambda_inj = float(lambda_inj)
        self.edge_filter = str(edge_filter)
    ...
    # ---  accept edge and pass to backbone   ---
    def forward(self, x, timesteps, cond_img, edge=None):
        features = self.backbone(
            x, timesteps, cond_img,
            edge=edge,
            lambda_inj=self.lambda_inj,
    ...
    def sample_unet(self, x, timesteps, cond_img, edge=None):
        return self.forward(x, timesteps, cond_img, edge=edge)

    def extract_features(self, cond_img, edge=None):
        # do nothing (keep compatibility with your diffusion wrapper)
        return cond_img
